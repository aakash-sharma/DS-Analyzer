...
for arch in 'vgg11'; do
	    ...
	    echo "$batch $arch"
	    echo "==============================================="
			python3 -u harness.py --nproc_per_node=4 -j 16 -a $arch --nnodes=1 -b $batch --steps RUN0 RUN1 --resume_dir results/results-p3/p3.8xlarge/dali-cpu/  image_classification/pytorch-imagenet-dali-mp.py 
