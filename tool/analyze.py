...
def compare_instances(result_dir):
    ...
    for X in [X_large, X_small]:
        ...
        for batch in BATCH_SIZES:
            ...
            axs1[0].set_xticklabels(X, fontsize=10)
            axs1[0].set_xlabel("Models", fontsize=10)
            axs1[0].set_ylabel("Percentage", fontsize=10)
            axs1[0].set_title("Prep stall comparison", fontsize=10)
...
def compare_models(result_dir):
    ...
    for model in models:
        ...
        for i in range(len(instances)):
            axs3[0].bar(X_BAT_axis - WIDTH + diff, Y_GPU_UTIL_CACHED_PCT_LIST[i], 0.2, label=instances[i])
            add_text(X_BAT_axis - 0.25 + diff, Y_GPU_UTIL_CACHED_PCT_LIST[i], axs3[0])
            axs3[1].bar(X_BAT_axis - WIDTH + diff, Y_GPU_MEM_UTIL_CACHED_PCT_LIST[i], 0.2, label=instances[i])
